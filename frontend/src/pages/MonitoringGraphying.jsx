...
const BOXES = [
  { title: "GR3 To Pharmacy", ip: "88.88.88.9", ifIndex: 5, accent: "green" },
  { title: "RBGR3-UPLINK MIMOSA C5C", ip: "88.88.88.10", ifIndex: 5, accent: "blue" }, // ?? ?????? ????
  { title: "TP-Link Switch 1", ip: "88.88.88.254", ifIndex: 49179, accent: "green" },
  { title: "TP-Link Switch 2", ip: "10.88.88.254", ifIndex: 49162, accent: "green" },
  { title: "Link To Dragon Club", ip: "88.88.88.6", ifIndex: 3, accent: "green" },
  { title: "RB Gr3 To Ghanem Market", ip: "88.88.88.12", ifIndex: 3, accent: "green" },
  { title: "RB Gr3 To My Home", ip: "88.88.88.5", ifIndex: 5, accent: "green" },
  { title: "Link To My Office", ip: "88.88.88.7", ifIndex: 2, accent: "green" },
];
...
function Card({ item }) {
  ...
  return (
    <div style={{ ...s.card, ...sBorder[border], ...(pulse ? s.cardPulse : null) }}>
      ...
      <div style={s.lane}>
        <div style={s.laneTop}>
          <div style={s.laneK}>RATE</div>
          <div style={s.laneV}>RX / TX</div>
        </div>
        ...
        <div style={s.rateRow}>
          <div style={s.rateLine}>
            ...
            <div style={{ ...s.rateChip, ...(sBand[bandRx]?.chip || sBand.neutral.chip) }}>
              {fmt(rxNow)} <span style={s.rateUnit}>Mbps</span>
            </div>
          </div>
          <div style={s.rateLine}>
            <div style={s.rateK}>TX</div>
            <div style={{ ...s.rateChip, ...(sBand[bandTx]?.chip || sBand.neutral.chip) }}>
              {fmt(txNow)} <span style={s.rateUnit}>Mbps</span>
            </div>
          </div>
        </div>
...
export default function MonitoringGraphying() {
  ...
  async function fetchOne(b, now) {
    ...
    // Accept multiple possible key names (defensive)
  ...
  const okN = list.filter((x) => x.ok === true).length;
  const errN = list.filter((x) => x.ok === false).length;
  const waitN = list.filter((x) => x.ok === null).length;

  const allRx = list.flatMap((x) => x.rx || []);
  const allTx = list.flatMap((x) => x.tx || []);
    ...
    <div style={s.page}>
      ...
      <div style={s.inner}>
        <div className="noc-topbar" style={s.topBar}>
          <div style={s.brand}>
            <div style={s.h1}>Monitoring Graphying</div>
            <div style={s.h2}>NOC Pulse  Live Traffic RX/TX Mbps  {POLL_MS / 1000}s cadence</div>
          </div>
        ...
        <div className="noc-summary" style={s.summaryRow}>
          <SumCard k="Nodes" v={`${list.length}`} sub={`${okN} ok  ${errN} err  ${waitN} wait`} band={errN > 0 ? "bad" : okN > 0 ? "good" : "neutral"} />
...
const s = {
  ...
  lane: { padding: "10px 12px 12px 12px" },
  ...
  rateLine: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flex: 1 },
  ...
  footerNote: { marginTop: 10, fontSize: 11, fontWeight: 850, color: "rgba(226,232,240,0.62)" },

  err: { marginTop: 10, padding: 10, borderRadius: 14, background: "rgba(239,68,68,0.10)", border: "1px solid rgba(239,68,68,0.28)", color: "rgba(254,202,202,0.95)", fontSize: 12, fontWeight: 850 },
};
