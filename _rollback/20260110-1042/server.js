...
app.post("/auth/login", async (req, res) => {
  ...
  const token = jwt.sign({ sub: u.id, username: u.username, role: u.role }, secret, { expiresIn: "2h" });
...
const PING_ALLOWLIST_FILE = "data/ping_allowlist.json";
let _pingAllowCache = { mtime: 0, ips: new Set(), cidr: [] };

  /* ===== TRAFFIC allowlist (target IPs) ===== */
  const TRAFFIC_ALLOWLIST_FILE = "data/traffic_allowlist.json";
